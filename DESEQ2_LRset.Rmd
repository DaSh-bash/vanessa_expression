...
par(mfrow=c(1,1))
# Make a basic volcano plot
with(res05abf, plot(log2FoldChange, -log10(pvalue), pch=20, col="grey", main="Abdomen", xlim=c(-20,20), ylim=c(0,15)))
...
#res05abfdata$TestGeneID <- res05abffuncdata$GeneID

p <- ggplot(data=res05abfdata, aes(x=log2FoldChange, y=-log10(pvalue), col=diffexpressed, label = delabel)) + geom_point() + theme_classic() +         geom_text_repel() + ylim(0, 20) + theme(aspect.ratio=1/3)


p2 <- p + geom_vline(xintercept=c(-2, 2), col="black") #+ geom_hline(yintercept=-log10(0.05), col="black")

p3 <- p2 + scale_color_manual(values=c("#062A79", "grey", "#CB0815"))
p3
...
p <- ggplot(data=res05hef2data, aes(x=log2FoldChange, y=-log10(padj), col=diffexpressed, label = delabel)) + geom_point() + theme_classic() + geom_text_repel(data=df, aes(x=log2FoldChange, y=-log10(padj), col=diffexpressed, label = delabel)) +ylim(0, 20) + geom_vline(xintercept=c(-2, 2), col="black")  + scale_color_manual(values=c("#062A79", "grey", "#CB0815"))

p

px <- ggplot(data=df, aes(x=log2FoldChange, y=-log10(padj), col=diffexpressed, label = delabel)) + geom_point() + theme_classic() + geom_text_repel() +ylim(0, 20) + geom_vline(xintercept=c(-2, 2), col="black")  + scale_color_manual(values=c("#062A79", "grey", "#CB0815"))

px
#ggsave("res05hef2data.pdf")
...
```{r}
df <-  res05hef2data %>%
  filter(log2FoldChange > 5 & diffexpressed != "NO" | -log10(padj) > 10) #| (log2FoldChange < 45 & flipper_length_mm <180) )
...
```{r}
#gs.RNASeq <- 18860
allgenes_count <- 18860
go.obj <- newGeneOverlap(row.names(res05hef2topDown),
                          row.names(res05abftopDown),
...
data2 <- data.frame(x1 = 1:5,    # Create example data
                   x2 = 6:10,
                   x3 = 11:15)
...
# Stacked
kegg_adults <- ggplot(data, aes(fill=condition, y=gene_ratio_kegg, x=kegg_name)) + 
    geom_bar(position="stack", stat="identity", fill="red") +
  scale_x_discrete(limits=kegg_name) + theme(panel.background = element_blank(),
        strip.background = element_blank(),
        strip.text = element_blank(),
        axis.line = element_line(size = 0.2, colour = "black"),
        axis.text.x = element_text(size = 8, angle = 90, vjust = 0.5, hjust = 1),
